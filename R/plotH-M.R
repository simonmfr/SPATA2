...
#' two ids with identical dimensions. Set merge_ias=TRUE and id2="name_of_id2". 
#'
#' @inherit imageAnnotationScreening params details
#' @inherit plotIasLineplot params
#' @inherit plotTrajectoryHeatmap params
...
plotIasHeatmap_merge <- function(object,
                           id,
                           variables,
                           distance = NA_integer_,
                           n_bins_circle = NA_integer_,
...
# 1. Control --------------------------------------------------------------
  ...
  # option: merge multiple annotations

  if(base::isTRUE(merge_ias)){
    
    if (!id2 %in% names(getImageAnnotations(visium))) { print(paste0("id2 not defined or not available in this spata2 object"))
    } else {
    ...
    img_ann_df2 <-
        ...
        # get mean of id1 and id2
        m <- merge(img_ann_df, img_ann_df2, by=c("bins_circle","bins_order","variables"))
        m <- m %>% mutate(values = rowMeans(.[, c("values.x","values.y")]))
        m <- dplyr::select(m, bins_circle, bins_order, variables, values)
        m <- dplyr::arrange(m, bins_order)
        # note: order of variables changed! not sure if matters.        
